...
<div class="h-full flex bg-gray-950">
	<!-- Sidebar: Project List -->
	<aside class="w-56 shrink-0 border-r border-gray-800 overflow-y-auto bg-gray-900/50">
		<div class="p-4">
			...
			<nav class="space-y-1">
				{#each data.projects as project}
					...
					<a
						...
						class="block px-3 py-2 rounded-lg text-sm transition-colors {isActive
		...
		{#if data.selectedProject && selectedProjectData}
			<div class="max-w-3xl">
				<div class="flex items-center justify-between mb-6">
					<div>
						<h2 class="text-2xl font-bold text-white">{data.selectedProject}</h2>
					...
					>
						...
						<button
							...
							class="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-600 rounded-lg text-sm text-white transition-colors"
						>
				...
				{#if form?.error}
					<div class="mb-6 p-4 bg-red-900/30 border border-red-800 rounded-lg text-red-300">
						{form.error}
					</div>
				{/if}
							...
							<div class="mt-3 pt-3 border-t border-gray-700">
								...
								<div class="flex flex-wrap gap-2">
									{#each selectedProjectData.repoInfo.workflowFiles as file}
										<span class="px-2 py-0.5 bg-gray-700 rounded text-gray-300 text-xs">{file}</span>
									{/each}
								</div>
							</div>
						{/if}
				...
				<div class="grid gap-6">
								...
								<div>
									...
									<div class="flex flex-wrap gap-2 mt-1">
										{#if selectedProjectData.infraConfig.stack.framework}
											<span class="px-2 py-1 bg-purple-900/50 rounded text-purple-300 text-xs">
												{selectedProjectData.infraConfig.stack.framework}
											</span>
										{/if}
										{#if selectedProjectData.infraConfig.stack.language}
											<span class="px-2 py-1 bg-blue-900/50 rounded text-blue-300 text-xs">
												{selectedProjectData.infraConfig.stack.language}
											</span>
										{/if}
										{#each selectedProjectData.infraConfig.stack.css || [] as css}
											<span class="px-2 py-1 bg-cyan-900/50 rounded text-cyan-300 text-xs">
												{css}
											</span>
										{/each}
										{#each selectedProjectData.infraConfig.stack.testing || [] as test}
											<span class="px-2 py-1 bg-green-900/50 rounded text-green-300 text-xs">
												{test}
											</span>
										{/each}
										{#if selectedProjectData.infraConfig.stack.buildTool}
											<span class="px-2 py-1 bg-orange-900/50 rounded text-orange-300 text-xs">
					...
					{#if displayStack}
						<div class="bg-gray-800/50 border border-gray-700 rounded-lg p-6">
							<div class="flex items-center gap-2 mb-4">
								...
								<h3 class="text-lg font-semibold text-white">Detected from package.json</h3>
							</div>
								...
								<div>
									...
									<div class="flex flex-wrap gap-2 mt-1">
										...
										{#if displayStack.icons}
											<span class="px-2 py-1 bg-pink-900/50 rounded text-pink-300 text-xs">
												{displayStack.icons}
											</span>
										{/if}
									</div>
								...
								{#if displayPackageJson}
									<div>
										<div class="flex items-center justify-between mb-1">
											...
											>
												...
												<button
													...
													class="flex items-center gap-1 px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 disabled:bg-gray-600 rounded text-gray-300 transition-colors cursor-pointer"
												>
													{#if checkingOutdated}
										...
										{#if outdatedResults}
											...
											<div class="mb-2 p-2 bg-gray-900 rounded text-xs">
														...
														{#each outdated as pkg}
															<div class="flex items-center gap-2 {pkg.majorBehind > 0 ? 'text-red-400' : 'text-yellow-400'}">
																...
																{#if pkg.majorBehind > 0}
																	<span class="px-1 bg-red-900/50 rounded text-red-300 text-xs">+{pkg.majorBehind} major</span>
																{/if}
															</div>
														{/each}
													</div>
										...
										<div class="text-xs text-gray-400 max-h-48 overflow-auto bg-gray-900 rounded p-2">
					...
					{:else if !selectedProjectData.infraConfig}
						<div class="bg-gray-800/50 border border-gray-700 rounded-lg p-6">
							<div class="flex items-center gap-2 mb-4">
								<Package class="w-5 h-5 text-gray-400" />
								<h3 class="text-lg font-semibold text-white">Auto-detect Tech Stack</h3>
							</div>
