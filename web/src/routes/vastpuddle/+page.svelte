<script lang="ts">
	...
	// View mode: 'repos' shows source repos with instances, 'ownership' shows clients with org portals and campuses, 'flat' shows all campuses
	...
	// For flat view - all campuses across all clients
	let allCampuses = $derived(
		filteredClients.flatMap(client =>
			client.campuses.map(campus => ({
				...campus,
	...
	let sortedCampuses = $derived(
		sortAsc
			? [...allCampuses].sort((a, b) => a.displayName.localeCompare(b.displayName))
			: [...allCampuses].sort((a, b) => b.displayName.localeCompare(a.displayName))
	);
	...
	// For flat view
	const FLAT_CARD_WIDTH = 280;
	const FLAT_CARD_HEIGHT = 100;
	const FLAT_GAP = 20;
	...
	function getContentDimensions() {
		if (viewMode === 'ownership') {
			...
			// Flat view
			const rows = Math.ceil(sortedCampuses.length / FLAT_COLUMNS);
			return {
				width: PADDING + FLAT_COLUMNS * (FLAT_CARD_WIDTH + FLAT_GAP) - FLAT_GAP + PADDING,
				height: PADDING + rows * (FLAT_CARD_HEIGHT + FLAT_GAP) - FLAT_GAP + PADDING
			};
		}
	}
...
<svelte:head>
	<title>Vast Puddle | Orchon</title>
</svelte:head>
...
<div class="h-full flex flex-col bg-gray-950 overflow-hidden">
	...
	<div class="shrink-0 flex items-center justify-between px-4 py-2 bg-gray-900/80 border-b border-gray-800 backdrop-blur-sm z-10">
		<div class="flex items-center gap-4">
			<h1 class="text-lg font-semibold text-white">Vast Puddle</h1>
	...
	<div class="shrink-0 px-4 py-1 text-xs text-gray-500 bg-gray-900/50">
		Scroll to zoom | Drag to pan | {viewMode === 'ownership' ? 'Client ownership hierarchy' : viewMode === 'repos' ? 'Source repos with deployed instances' : 'All campuses'}
	</div>
	...
	>
		...
		>
			...
			{:else}
				<!-- Flat view - grid of all campuses -->
				{#each sortedCampuses as campus, idx (campus.id)}
					{@const col = idx % FLAT_COLUMNS}
					{@const row = Math.floor(idx / FLAT_COLUMNS)}
					{@const x = PADDING + col * (FLAT_CARD_WIDTH + FLAT_GAP)}
					{@const y = PADDING + row * (FLAT_CARD_HEIGHT + FLAT_GAP)}
					<div
						class="absolute bg-gray-900 border border-gray-600 rounded-lg overflow-hidden shadow-lg hover:border-gray-500 transition-colors"
						style="left: {x}px; top: {y}px; width: {FLAT_CARD_WIDTH}px; height: {FLAT_CARD_HEIGHT}px;"
					>
						<div class="px-3 py-2 bg-gray-800/50 border-b border-gray-700">
							<div class="flex items-center gap-2">
								<School class="w-3.5 h-3.5 text-blue-400" />
								<span class="font-medium text-white text-sm truncate">{campus.displayName}</span>
							</div>
							<div class="text-[10px] text-gray-500 mt-0.5">{campus.clientName}</div>
						</div>
						<div class="px-3 py-2">
							{#if campus.productionUrl}
								<a
		...
		<div class="flex items-center gap-6 flex-wrap">
			...
			{:else}
				<div class="flex items-center gap-3">
					...
					<div class="flex items-center gap-1.5">
						<School class="w-3.5 h-3.5 text-blue-400" />
						<span class="text-xs text-gray-400">All Campuses</span>
					</div>
				</div>
			{/if}
		</div>
