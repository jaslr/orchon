...
export async function runUptimeChecks(): Promise<void> {
  ...
  for (const project of projectsWithUrls) {
    ...
    if (wasUp !== undefined && wasUp !== result.isUp) {
      ...
      if (!result.isUp) {
        await dispatchAlert(
          ...
          hostingService.id,
          'down',
          `${project.displayName} is down! ${result.errorMessage || `HTTP ${result.statusCode}`}`
        );
      }
    }
    ...
    console.log(
      `Uptime check: ${project.id} - ${result.isUp ? 'UP' : 'DOWN'} (${result.responseTimeMs}ms)`
    );
  }
