 ...
 */
async function checkDatabaseHealth(projectRef: string): Promise<HealthCheckResult> {
 ...
 */
async function checkAuthHealth(projectRef: string): Promise<HealthCheckResult> {
...
function mapHealthToStatus(result: HealthCheckResult): 'healthy' | 'degraded' | 'down' {
  if (!result.isHealthy) return 'down';
  if (result.responseTimeMs && result.responseTimeMs > 3000) return 'degraded';
  return 'healthy';
}

export async function pollSupabaseServices(): Promise<void> {
  console.log('Polling Supabase services...');

  // Find projects that use Supabase
  const supabaseProjects = projects.filter((p) =>
    p.services.some((s) => s.provider === 'supabase')
  );
        ...
        if (status !== prevStatus) {
          ...
          if (status === 'down' || (prevStatus === 'healthy' && status === 'degraded')) {
            await dispatchAlert(
              ...
              `Supabase database for ${project.displayName} is ${status}: ${result.message}`
            );
          }
        }
      }
        ...
        if (status !== prevStatus) {
          ...
          if (status === 'down' || (prevStatus === 'healthy' && status === 'degraded')) {
            await dispatchAlert(
              ...
              service.id,
              status === 'down' ? 'down' : 'degraded',
              `Supabase auth for ${project.displayName} is ${status}: ${result.message}`
            );
          }
        }
      }
