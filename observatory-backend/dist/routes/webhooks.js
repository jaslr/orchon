...
webhookRoutes.post('/github', async (c) => {
    ...
    const expectedSignature = 'sha256=' + crypto.createHmac('sha256', env.githubWebhookSecret).update(body).digest('hex');
    try {
    ...
    try {
        ...
        if (result) {
            ...
            if (result.status === 'failure') {
                await dispatchAlert(result.projectId, result.serviceId, 'deploy_failure', `Workflow "${result.workflowName}" failed on ${result.projectId}`);
            }
