...
export async function pollFlyioApps() {
    ...
    const flyProjects = projects.filter((p) => p.services.some((s) => s.provider === 'flyio'));
    for (const project of flyProjects) {
        ...
        const app = apps.find((a) => a.name.toLowerCase() === project.name.toLowerCase() ||
        ...
        // Store status check
        try {
            await db.insertStatusCheck(flyService.id, status, `Machines: ${app.machines.nodes.filter((m) => m.state === 'started').length}/${app.machines.nodes.length} running`);
        }
        catch (err) {
        ...
        if (status !== prevStatus) {
            ...
            if (status === 'down' || (prevStatus === 'healthy' && status === 'degraded')) {
                await dispatchAlert(project.id, flyService.id, status === 'down' ? 'down' : 'degraded', `Fly.io app ${app.name} is ${status}`);
            }
        }
    }
