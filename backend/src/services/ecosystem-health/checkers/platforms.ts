...
async function checkFlyioBackend(): Promise<CheckResult> {
...
async function checkFlyioDb(): Promise<CheckResult> {
  const start = Date.now();
...
async function checkCloudflarePages(): Promise<CheckResult> {
...
export async function checkPlatformServices(): Promise<EcosystemNode[]> {
  ...
  const flyioStatus =
    backendResult.status === 'healthy' && dbResult.status === 'healthy'
      ? 'healthy'
  ...
  const cfStatus =
    pagesResult.status === 'healthy' ? 'healthy' : pagesResult.status;
