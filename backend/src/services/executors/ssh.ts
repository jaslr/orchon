...
export async function executeSSHCommand(config: Record<string, unknown>): Promise<string> {
  const { host, command, user = 'root', port = 22 } = config as unknown as SSHConfig;
