...
export async function executeGCPAction(config: Record<string, unknown>): Promise<string> {
  const { project, service, region = 'australia-southeast1', action = 'restart' } = config as unknown as GCPConfig;
  ...
  const serviceData = await getResponse.json() as { template?: { revision?: string; annotations?: Record<string, string> }; conditions?: Array<{ type: string }> };
  ...
  const result = await updateResponse.json() as { template?: { revision?: string }; conditions?: Array<{ type: string }> };
