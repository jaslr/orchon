...
// =============================================================================
// Recovery Actions (Infrastructure Control)
// =============================================================================
...
export async function getAllRecoveryActions(): Promise<RecoveryAction[]> {
  const result = await query<RecoveryAction>(`
    SELECT
      id,
      service_id as "serviceId",
      name,
...
export async function getRecoveryActionsByService(serviceId: string): Promise<RecoveryAction[]> {
  const result = await query<RecoveryAction>(
    `
    SELECT
      id,
...
export async function createActionExecution(execution: {
  ...
  const result = await query<ActionExecution>(
    `
...
export async function updateActionExecution(
  id: string,
  updates: { status: ActionExecution['status']; output?: string }
...
export async function getRecentExecutions(limit: number): Promise<(ActionExecution & { actionName: string; serviceId: string })[]> {
  const result = await query<ActionExecution & { actionName: string; serviceId: string }>(
    `
    SELECT
      e.id,
