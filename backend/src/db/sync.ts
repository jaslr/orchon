...
export async function syncProjectsToDb(): Promise<void> {
  ...
  for (const project of projects) {
    try {
      ...
      for (const service of project.services) {
        ...
        if (service.cfProjectName) config.cfProjectName = service.cfProjectName;
        if (service.flyAppName) config.flyAppName = service.flyAppName;
        if (service.gcpProjectId) config.gcpProjectId = service.gcpProjectId;
        if (service.supabaseProjectRef) config.supabaseProjectRef = service.supabaseProjectRef;
