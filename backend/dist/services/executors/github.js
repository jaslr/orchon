...
export async function executeGitHubWorkflow(config) {
    const { owner, repo, workflow, ref = 'main', inputs = {} } = config;
    ...
    const runsResponse = await fetch(`https://api.github.com/repos/${owner}/${repo}/actions/workflows/${workflow}/runs?per_page=1`, {
        headers: {
            Authorization: `Bearer ${token}`,
            Accept: 'application/vnd.github.v3+json',
            'User-Agent': 'ORCHON-Executor',
